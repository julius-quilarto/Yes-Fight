...
public class ThiccTanuki {

	/**
	 * A class designed for how the player will control and interract with the 
	 * Thicc Tanuki character and how their base stats work.
	 * version 2.3
	 */
	...
	private final int HPbase = 100;//she's thicc so she has a high base HP stat
	private final int AtkBase = 60;
	private final int DefBase = 80;
	private final int SpecBase = 120;//she's smart so she has a high special
	private final int AGILITY= 25;//maximum value is 100 which would mean she dodges everything
	private final String FIRST_NAME = "Tania";
	private final String LAST_NAME = "Nooxie";
	private final String NICKNAME = "Thicc Tanuki";
	private final int BASE_EXP = 100;//base exp for leling up
	private final int MAX_MAGIC = 120;//a stat that is used for the maximum number of magic points a character can have
	private final Abilities ability = Abilities.TanukiSense;//sets the unchanged default ability of the character
	private final char GENDER = 'F';//'M' for male and 'F' for female
	private final Races race = Races.tanuki;//character race in case it become relevant later
	private final String[] TAUNTS = new String[] {
			"THICC TANUKI: Hey look! Your stats suck!",
			"THICC TANUKI: Can you actually use any good moves?",
			"THICC TANUKI: You suck so much you make me want to delete System32!",
			"THICC TANUKI: A miniature dachshund is more threatening than you.",
			"THICC TANUKI: You're skills are as real as Half Life 3.",
			"THICC TANUKI: Do you honestly think you're funny? Seriosly!",
			"THICC TANUKI: UP, UP, DOWN, DOWN, LEFT, RIGHT, LEFT, RIGHT, B, A, Time to thrash you until there's nothing left to thrash!",
			"THICC TANUKI: Skrub."
	};
	...
	private Moves move1 = Moves.shootGun;
	private Moves move2 = Moves.pummel;
	private Moves move3 = Moves.shadowPunch;
	private Moves move4 = Moves.rockThrow;
	private Spells spell1 = Spells.TanukiEvade;
	private Spells spell2 = Spells.TanukiTeamEvade;
	private Spells spell3 = Spells.LookKawaii;
	private Spells spell4 = Spells.none;
	private Moves superMove = Moves.TanukiIllusions;//Thicc Tanuki's signature super move
	private Weapons weapon = Weapons.colt1911;//defaults to her signature Colt 1911
	 ...
	 */
	public ThiccTanuki(int currentHP,int level,Armor a, Weapons w, int exp, Moves[] moveSet, Spells[] spellSet) {
		this.HP = currentHP; 
		if(currentHP < 1) {
			this.HP = 0;
			this.isAlive = false;
	...
	public Moves chooseMove(int moveNumber) {
		this.magicMeter = this.magicMeter + 15;//restores magic meter by using regular attacks
		this.superPowerMeter += 15;
		if(superPowerMeter > 100)superPowerMeter = 100;
		...
		else if(moveNumber == 5 && superPowerMeter > 99) {
			superPowerMeter = 0;//needs looking at because the super meter doesn't seem to reset after using a supermove
	...
	public Spells chooseSpell(byte selection) {
		if(selection == 1 && magicMeter >= SpellsData.TanukiEvadeCST) {
			this.magicMeter = magicMeter - SpellsData.TanukiEvadeCST;
			return spell1;
		}
		else if(selection == 2 && magicMeter >= SpellsData.TanukiTeamEvadeCST) {
			this.magicMeter = magicMeter - SpellsData.TanukiTeamEvadeCST;
			return spell2;
		}
	...
	public void giveWeapon(Weapons w) {
		...
		if (w == Weapons.colt1911) {
			...
			this.Atk = (int)(this.Atk * 2.1);//45-70 lever gun has quick a bit of power and kick in real life and video games
		}
	}
